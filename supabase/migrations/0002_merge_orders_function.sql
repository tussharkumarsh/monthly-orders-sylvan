-- Transactional merge of uploaded Excel rows into the orders table.
-- A function body runs in a single implicit transaction in Postgres,
-- so if any row raises an exception, all changes in this call are
-- rolled back (all-or-nothing upload).
...
begin
  ...
  loop
    ...
    select * into existing from orders
      where orders.order_no = new_order_no and deleted_at is null
      for update;
    ...
    if not found then
      insert into orders (
        ...
        shipping_cost, packing_dimension, packing_weight
      ) values (
        ...
        new_shipping_cost, new_packing_dimension, new_packing_weight
      );
      ...
      if existing.date is distinct from new_date
        ...
        or existing.product_name is distinct from new_product_name
      ...
      then
        update orders set
          ...
          -- shipping_cost, packing_dimension, packing_weight are preserved
          -- from the existing record and NEVER overwritten by re-upload.
